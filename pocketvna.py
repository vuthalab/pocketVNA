...
class Driver:
    ...
    def scan_WithNumpy(self, freqs, avg, netparams, callback = None):
        if len(freqs) < 1: return [numpy.array([], dtype=complex128)] * 4
        if not bool(self.handle): return [None]*4
    ...
    def scan_skrf_network(self, freq, avg):
        if not SKRF: raise RuntimeError("skrf (scikit-rf) library was not imported.")

        s11, s21, s12, s22 = self.scan(freq, avg,  NetworkParams.AllSupported)
        ...
        #       [s21, s22]]
        #    ]
        #    ntwk.s = numpy.array([  [s11, s21], [s12, s22]  ])
